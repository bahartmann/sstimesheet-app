...
function VirtualScroller(options) {
    ...
    var pageCount = options.infinite ? 5 : Math.min(options.itemCount, 5);
    ...
    function getPageFromIndex(vIndex) {
        if (options.infinite || options.itemCount > 5) {
            // First two pages
            if (vIndex < 2) {
                return vIndex;
            }
    ...
    if (options.infinite || options.itemCount > 5) {
        ...
        // If there are less than 5 items our scroll logic is significantly simpler
        for (var k = 0; k < pageCount; k++) {
            loadView(containers[k], k);
        }

        scrollEndListener = function (e) {
            currentVirtualIndex = containers[e.currentPage].vIndex;
        };
